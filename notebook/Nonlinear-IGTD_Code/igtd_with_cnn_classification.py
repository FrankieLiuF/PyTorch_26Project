# -*- coding: utf-8 -*-
"""IGTD_with_CNN_classification.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/12vjcxsYMgnYDW8CU6mHYGGlhadCe2PHC

# IGTD
"""

from scipy.stats import spearmanr, rankdata, wasserstein_distance
...
def generate_image_data(data, index, num_row, num_column, coord, image_folder=None, file_name=''):
    ...
    if isinstance(data, pd.DataFrame):
        samples = data.index.map(np.str_)
        data = data.values
    else:
    ...
    for i in range(data_2.shape[0]):
        ...
        if image_folder is not None:
            ...
            plt.axis('scaled')
            plt.savefig(fname=image_folder + '/' + file_name + '_' + samples[i] + '_image.pdf', bbox_inches='tight',
                        pad_inches=0)
            plt.close(fig)
...
def table_to_image(norm_d, scale, fea_dist_method, image_dist_method, save_image_size, max_step, val_step, normDir,
    ...
    plt.savefig(fname=normDir + '/original_feature_ranking.pdf', bbox_inches='tight', pad_inches=0)
    plt.close(fig)
    ...
    plt.savefig(fname=normDir + '/image_ranking.pdf', bbox_inches='tight', pad_inches=0)
    plt.close(fig)
    ...
    fig = plt.figure()
    plt.plot(time, err)
    plt.savefig(fname=normDir + '/error_and_runtime.pdf', bbox_inches='tight', pad_inches=0)
    plt.close(fig)
    fig = plt.figure()
    plt.plot(range(len(err)), err)
    plt.savefig(fname=normDir + '/error_and_iteration.pdf', bbox_inches='tight', pad_inches=0)
    plt.close(fig)
    ...
    plt.savefig(fname=normDir + '/optimized_feature_ranking.pdf', bbox_inches='tight', pad_inches=0)
    plt.close(fig)
...
num_row = 105    # Number of pixel rows in image representation
num_col = 105    # Number of pixel columns in image representation
...
data = pd.read_csv('/content/drive/MyDrive/IGTD/Ovarian/data.csv', header = None, index_col = 0)
...
error = 'squared'
result_dir = '/content/drive/MyDrive/IGTD/Ovarian/Euclidean'
...
# 儲存結果到檔案
with open(os.path.join(result_dir, "execution_time.txt"), "w") as f:
...
# 定義參數
fea_dist_method = 'Geodesic'
image_dist_method = 'Euclidean'
error = 'squared'
result_dir = '/content/drive/MyDrive/IGTD/Ovarian/Geodesic'
os.makedirs(name=result_dir, exist_ok=True)
...
image_dist_method = 'Euclidean'
error = 'squared'
result_dir = '/content/drive/MyDrive/IGTD/Ovarian/Pearson'
os.makedirs(name=result_dir, exist_ok=True)
...
# 定義參數
fea_dist_method = 'Jensen'
image_dist_method = 'Euclidean'
error = 'squared'
result_dir = '/content/drive/MyDrive/IGTD/Ovarian/Jensen'
os.makedirs(name=result_dir, exist_ok=True)
...
# 定義參數
fea_dist_method = 'Wasserstein'
image_dist_method = 'Euclidean'
error = 'squared'
result_dir = '/content/drive/MyDrive/IGTD/Ovarian/Wasserstein'
os.makedirs(name=result_dir, exist_ok=True)
...
image_dist_method = 'Euclidean'
error = 'squared'
result_dir = '/content/drive/MyDrive/IGTD/Ovarian/Tropical'
os.makedirs(name=result_dir, exist_ok=True)
...
def load_data(path, method, dim):
  res = pd.read_csv(path + 'response.txt', header = 0, sep = '\t')
  res = res['Classifier']
  sample = []
  id = []

  route = os.path.join(path + method + '/TXT/')
  file = os.listdir(route)

  image = np.empty((len(file), dim, dim, 1))

  for i in range(len(file)):
        if file[i].split('.')[1] == 'txt' :
            id.append(i)
            data = pd.read_csv(path + method + '/TXT/' + file[i], engine='c',
                               na_values=['na', '-', ''], header = None, sep = '\t')
            image[i, :, :, 0] = data.values
            sample.append(file[i].split('.txt')[0])
  image = image[id, :, :, :]
  gene = {}
  gene['data'] = image
  gene['sample'] = sample
...
ns = [254]
method = ['Euclidean','Pearson','Geodesic','Jensen','Wasserstein','Tropical']
dim = 105

for n in ns:
    for m in method:
        path = '/content/drive/MyDrive/IGTD/Ovarian/'
        method_name = m
        res, gene = load_data(path=path, method=method_name, dim=dim)
        ...
        for train_index, val_index in kfold.split(X, y):
            ...
            y_train, y_val = y[train_index], y[val_index]
                ...
                # 建立 CNN 模型
            model = create_cnn(input_shape=(shape, shape, 1))
            ...
            reduce_lr = ReduceLROnPlateau(
                    ...
                    factor=0.1,
                    patience=10,
                    min_delta=validation_loss_threshold
                )
            ...
            # 計算性能指標
            accuracy_list.append(accuracy_score(y_val, y_pred_labels))
            precision_list.append(precision_score(y_val, y_pred_labels, average='binary'))
            recall_list.append(recall_score(y_val, y_pred_labels, average='binary'))
            f1_list.append(f1_score(y_val, y_pred_labels, average='binary'))

            #損失補齊
            max_len = max(len(loss) for loss in all_train_loss)
            all_train_loss = [np.pad(loss, (0, max_len - len(loss)), 'constant',constant_values=np.mean(loss)) for loss in all_train_loss]
            all_val_loss = [np.pad(loss, (0, max_len - len(loss)), 'constant',constant_values=np.mean(loss)) for loss in all_val_loss]
        ...
        # 結果輸出
        print(f"Accuracy: Mean = {results['mean_accuracy']:.4f}, Std = {results['std_accuracy']:.4f}")
        print(f"Precision: Mean = {results['mean_precision']:.4f}, Std = {results['std_precision']:.4f}")
        print(f"Recall: Mean = {results['mean_recall']:.4f}, Std = {results['std_recall']:.4f}")
        print(f"F1 Score: Mean = {results['mean_f1']:.4f}, Std = {results['std_f1']:.4f}")

        # 繪製損失曲線
        plt.figure(figsize=(10, 6))
        plt.plot(np.mean(all_train_loss, axis=0), label='All Train Loss', color='blue')
        plt.plot(np.mean(all_val_loss, axis=0), label='All Val Loss', color='orange')
